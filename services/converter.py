...
from services.legacy.assets.functions.functions import to_hls, convert_ad
...
def convert_video():
    while True:
        ...
        try:
            to_convert = Video.objects.filter(Q(is_processing=False, server=settings.SERVER) & Q(m3u8=None)).order_by(
                "-channel__view")
        except:
        ...
        try:
            ...
            to_convert.is_processing = False
            try:
                to_convert.save()
